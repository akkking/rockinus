...
if(isset($_POST['content']))
{
 ...
 $sql_in= mysql_query("SELECT descrip,memoid FROM rockinus.memo_follow_info WHERE memoid='$memoid' ORDER BY memofid DESC");
 $object = mysql_fetch_object($sql_in);
 $descrip = $object->descrip; 
 }
 
 $q_memo = mysql_query("SELECT sender FROM rockinus.memo_info WHERE memoid='$memoid'");
 if(!$q_memo) die(mysql_error());
 $object_memo = mysql_fetch_object($q_memo);
 $recipient = $object_memo->sender;
 $recipient = $object_memo->sender;
...
?>
<table width="450" height="51" border="0" cellpadding="0" cellspacing="0" bgcolor="#F5F5F5" style="border-left:0px solid #EEEEEE; margin-top:5">
  <tr>
    <td width="466" height="20" align="left" valign="top" bgcolor="#F5F5F5" style="padding:5 10 5 5; font-size:12px;">
  ...
  <tr>
    <td height="23" colspan="2" valign="top" style="line-height:150%; font-size:13px; padding:0 10 5 5;"><?php
