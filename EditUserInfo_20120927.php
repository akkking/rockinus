        ...
        <table width="760" height="394" border="0" cellpadding="0" cellspacing="0" align="right">
          <tr>
            <td valign="top" align="right">
              ...
              <table width="740" height="564" border="0" cellpadding="0" cellspacing="0" style="border-left:#CCCCCC dotted 0;border-right:#CCCCCC dotted 0;border-bottom:#CCCCCC dotted 0;">
                ...
                <tr>
                  ...
                  <td height="50">&nbsp;
                      <input name="fname" type="text" class="box" style="font-size:14px; font-family:Verdana, Arial, Helvetica, sans-serif" size="13" value="<?php echo($fname); ?>" />
                    &nbsp;.&nbsp;
                      <input name="lname" type="text" class="box" style="font-size:14px; font-family:Verdana, Arial, Helvetica, sans-serif" size="13" value="<?php echo($lname); ?>" /></td>
                  <td height="50" colspan="2">&nbsp;</td>
                  </tr>
                <tr>
                ...
                <tr>
                  <td height="50" align="right" style="padding-right:15px; font-family:Verdana, Arial, Helvetica, sans-serif"><strong>I'm currently  </strong></td>
                  <td height="50" colspan="3" style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:14px">&nbsp;
           	...
           	<select name="fcountry" id="fcountry" onchange="regionChange(this);" style="font-size:14px; font-family:Verdana, Arial, Helvetica, sans-serif">
                ...
                <tr>
				  ...
				  <div style=" margin-bottom:0px; margin-top:2px; width:740" align="center">
			<?php 
			if(isset($_SESSION['rst_friend_flag']) && $_SESSION['rst_friend_flag']=="success"){
				...
				if(!$q) die(mysql_error());
				$hometown_no_row = mysql_num_rows($q);
				
				$hometown_limit= (isset($_GET["limit"])) ? $_GET["limit"] : 2;
				$hometown_page= (isset($_GET["page"]))? $_GET["page"] : 1;
				if((!$hometown_limit) || (is_numeric($hometown_limit) == false)|| ($hometown_limit < 2) || ($hometown_limit > 25)) 
				$hometown_limit = 1; //default
	
				if((!$hometown_page) || (is_numeric($hometown_page) == false) || ($hometown_page < 0) || ($hometown_page > $hometown_no_row))
				$hometown_page = 1; //default 
				
				$hometown_next_page = $hometown_page + 1;
				$hometown_div_id = "hometownDiv".$hometown_page;
				$hometown_total_pages = ceil($hometown_no_row / $hometown_limit);
				$hometown_set_limit = ($hometown_page * $hometown_limit) - $hometown_limit;
				
				if($hometown_no_row>0){
					echo "<div align='left' style='width:700; margin-bottom:15'><font style='font-size:14px; font-family: Verdana, Arial, Helvetica, sans-serif; font-weight:bold'>&nbsp;<img src=img/grayStar.jpg width=15 />&nbsp;&nbsp;&nbsp;Following student(s) also from your hometown :</div>";
					$q_hometown = mysql_query("SELECT * FROM rockinus.user_info WHERE fcountry = '$fcountry' AND fregion = '$fregion' AND uname<>'$uname' LIMIT $hometown_set_limit, $hometown_limit;");
					if(!$q_hometown) die(mysql_error());
					while($object = mysql_fetch_object($q_hometown)){					
		...
		$.ajax({
 		...
 		return false;
				...
				<?php
						...
						echo("<td style='padding-left:15px; padding-top:5; line-height:150%; font-family: Verdana, Arial, Helvetica, sans-serif;' valign='top'><a href='RockerDetail.php?uid=$loopname' target='_blank' class=one><font color=$_SESSION[hcolor]>$loopname</font></a><br><br>");
...
<script type="text/javascript">
function loadHometownFriend(pageNum,div_id)
{
//alert(pageNum);
//alert(div_id);
var xmlhttp;
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    ...
    $("#hometown_btn_<?php echo($hometown_div_id) ?>").hide();
	}
  }

xmlhttp.open("GET","loadFriend.php?page="+pageNum+"&proName=hometown&fcountry="+'<?php echo($fcountry) ?>'+"&fregion="+'<?php echo($fregion) ?>'+"&limit=2",true);
xmlhttp.setRequestHeader("Content-Type","text/html;charset=gb2312");
xmlhttp.send();
}
</script>
<?php
if($hometown_next_page <= $hometown_total_pages && isset($_SESSION['rst_flag']) ){ 
?>
<button type="button" id='hometown_btn_<?php echo($hometown_div_id) ?>' onClick="loadHometownFriend('<?php echo($hometown_next_page) ?>','<?php echo($hometown_div_id) ?>')" style="color:black; background-image:url(img/viewMore.png); font-size:14px; background-color: #F5F5F5; border:0px; margin-left:20; margin-bottom:20; height:29; width:130; font-weight:bold" onMouseOver="this.style.cursor='hand'"> 
&nbsp;
</button>
<?php 
} 
?>
<br>
<div id=<?php echo($hometown_div_id) ?>></div>
<br>
			...
			if(isset($_SESSION['rst_emotionGender_flag']) && $_SESSION['rst_emotionGender_flag']=="success"){
				...
				if(!$q) die(mysql_error());
				$emotion_no_row = mysql_num_rows($q);
				
				$emotion_limit= (isset($_GET["limit"])) ? $_GET["limit"] : 2;
				$emotion_page= (isset($_GET["page"]))? $_GET["page"] : 1;
				if((!$emotion_limit) || (is_numeric($emotion_limit) == false)|| ($emotion_limit < 2) || ($emotion_limit > 25)) 
				$emotion_limit = 1; //default
	
				if((!$emotion_page) || (is_numeric($emotion_page) == false) || ($emotion_page < 0) || ($emotion_page > $emotion_no_row))
				$emotion_page = 1; //default 
				
				$emotion_next_page = $emotion_page + 1;
				$emotion_div_id = "emotionDiv".$emotion_page;
				$emotion_total_pages = ceil($emotion_no_row / $emotion_limit);
				$emotion_set_limit = ($emotion_page * $emotion_limit) - $emotion_limit;

				$emotion_q = mysql_query("SELECT * FROM rockinus.user_info WHERE gender = '$gender_here' AND mstatus = '$mstatus' AND uname<>'$uname' LIMIT $emotion_set_limit, $emotion_limit;");
				if(!$emotion_q) die(mysql_error());
				$emotion_no_row = mysql_num_rows($emotion_q);
				if($emotion_no_row>0){
					echo "<div align='left' style='width:700; margin-bottom:15'><font style='font-size:14px; font-weight:bold; font-family: Verdana, Arial, Helvetica, sans-serif;'>&nbsp;<img src=img/grayStar.jpg width=15 />&nbsp;&nbsp;&nbsp;Following student(s) are also $mstatus :</div>";
					while($object = mysql_fetch_object($emotion_q)){					
						$loopname = $object->uname;
				?>
				
				<script type="text/javascript">
$(function() {
	$(".addFriendEmotionDiv<?php echo($loopname) ?>").click(function() {
		var sender = '<?php echo($uname) ?>';
		var recipient = '<?php echo($loopname) ?>';
		var dataString = 'sender='+sender+'&&recipient='+recipient; 
		//alert("dataString");
		
		$("#flashAddFriendEmotion<?php echo($loopname) ?>").hide();
		$("#addFriendEmotionResult<?php echo($loopname) ?>").show();
		$("#addFriendEmotionResult<?php echo($loopname) ?>").fadeIn(400).html('<img src="img/loading42.gif" align="absmiddle">');
		...
		$.ajax({
			...
			success: function(html){
				$("#flashAddFriendEmotion<?php echo($loopname) ?>").hide();
				$("#addFriendEmotionResult<?php echo($loopname) ?>").html(html);
				$("#addFriendEmotionResult<?php echo($loopname) ?>").show();
			}
 		});
 		return false;
				...
				<?php
						...
						if($rel_rstatus=="S")echo("<div style='font-size:14px; font-family:Verdana, Arial, Helvetica, sans-serif; font-weight:normal; width:85px; height:20; border:1px #DDDDDD solid; background: url(img/master.png); color:#333333;  padding:3 5 3 5; display:inline' align='center'><a href='EditUserInfo.php' class=one>+ Edit</a></div>&nbsp;"); 
							...
							}else if($rel_rstatus=="N")echo("<div id='addFriendEmotionDiv$loopname' class='addFriendEmotionDiv$loopname' style='font-size:14px; font-family:Verdana, Arial, Helvetica, sans-serif; font-weight:normal; width:85px; height:20; border:1px #DDDDDD solid; background: url(img/master.png); color:#333333;  padding:3 5 3 5; display:inline' onMouseOver=this.style.cursor='hand' align='center'>+ Friend</div>");
						?>
	<span id="flashAddFriendEmotion<?php echo($loopname) ?>" class="flashAddFriendEmotion<?php echo($loopname) ?>" style=" display:none; width:100; padding-right:5"></span>
 	 <span id="addFriendEmotionResult<?php echo($loopname) ?>" class="addFriendEmotionResult<?php echo($loopname) ?>" style='font-size:14px; font-family:Verdana, Arial, Helvetica, sans-serif; font-weight:normal; width:90; height:20; border:1px #DDDDDD solid; background: url(img/master.png); color:#333333;  padding:3 5 3 5; display:none' align='center'></span>&nbsp;
	 					<?php
	 					if($rel_rstatus!="S"){?><div style="font-size:14px; font-family:Verdana, Arial, Helvetica, sans-serif; font-weight:normal; width:90; height:20; border:1px #DDDDDD solid; background: url(img/master.png); color:#333333; padding-top:5; padding:3 5 3 5; display:inline" align="center"><a href="SendMessage.php?recipient=<?php echo($loopname) ?>" class='one'>Message</a>
					</div>
					<?php } 
			...
			<script type="text/javascript">
function loadEmotionFriend(pageNum,div_id)
{
//alert(pageNum);
//alert(div_id);
var xmlhttp;
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
    document.getElementById(div_id).innerHTML=xmlhttp.responseText;
    $("#emotion_btn_<?php echo($emotion_div_id) ?>").hide();
	}
  }

xmlhttp.open("GET","loadFriend.php?page="+pageNum+"&proName=emotion&mstatus="+'<?php echo($mstatus) ?>'+"&gender="+'<?php echo($gender) ?>'+"&limit=2",true);
xmlhttp.setRequestHeader("Content-Type","text/html;charset=gb2312");
xmlhttp.send();
}
</script>
<?php
if($emotion_next_page <= $emotion_total_pages && isset($_SESSION['rst_flag']) ){ 
?>
<button type="button" id='emotion_btn_<?php echo($emotion_div_id) ?>' onClick="loadEmotionFriend('<?php echo($emotion_next_page) ?>','<?php echo($emotion_div_id) ?>')" style="color:black; background-image:url(img/viewMore.png); font-size:14px; background-color: #F5F5F5; border:0px; margin-left:20; margin-bottom:20; height:29; width:130; font-weight:bold" onMouseOver="this.style.cursor='hand'"> 
&nbsp;
</button>
<?php 
} 
?>
<br>
<div id=<?php echo($emotion_div_id) ?>></div>
<br>			
				  </td>
                  </tr>
              </table>
