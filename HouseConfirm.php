...
if(isset($_POST['deleteHouse'])){
	...
	$q = mysql_query("DELETE FROM rockinus.house_info where hid=$hid");
	if(!$q) die(mysql_error());
	$_SESSION['rst_msg']="<div align='center' style='padding-top:10; padding-bottom:10; margin-top:10; color:#336633'><font size=4><strong><img src=img/addsuccessIcon.jpg>&nbsp;&nbsp; This House post has been deleted successfully!</strong><br><br><a href=".$pageName.".php class=one>Go Back</a></font></div>"; 
	//header("location:HouseFleaResult.php");
}else{
	if(isset($_GET['hid'])){
		$hid = $_GET['hid'];
		mysql_query("SET NAMES GBK");
		$q1 = mysql_query("SELECT * FROM rockinus.house_info where hid='$hid'");
  ...
  <table width="1024" height="394" border="0" cellpadding="0" cellspacing="0">
    <tr>
	  ...
	  <?php if(isset($_SESSION['rst_msg'])){
		...
		?>	
	  <form method="post" action="HouseConfirm.php">
      <input type="hidden" name="hid" value="<?php echo($hid) ?>" />
      <input type="hidden" name="pageName" value="<?php echo($pageName) ?>" />
	  <font style="font-size:20px">Are you sure, you want to delete this House post?</font><br /><hr size="2" style="margin-top:15" /><br />
	  <font style="font-size:16px; color:<?php echo($_SESSION['hcolor']) ?>"><strong><?php echo($subject) ?></strong></font>
	  <p style="margin-top:25" />
	  <div style=" padding-top:5">
	  <input type="submit" name="deleteHouse" value="Delete" style="background-color:#666666; padding-top:5; padding-left:5; padding-right:5; padding-bottom:5; height:25; border:0; color:#F5F5F5"> &nbsp;
