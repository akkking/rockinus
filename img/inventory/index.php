  ...
  <table width="1024" height="142" border="0" cellpadding="0" cellspacing="0">
    <tr>
     ...
     <?php
	  ...
	  <div align="center">
<!--<img src="img/GUnitThisis50New_com.png" width="183" height="183" />
-->
</div>
	  </form>
	  </td>
      <td width="791" valign="top"><div align="center">
        <table width="760" height="23" border="0" cellpadding="0" cellspacing="0" style="margin-bottom:5px; margin-top:0px; border-top:2px #000000 solid;">
          <tr>
            <td width="212" height="45" align="left" bgcolor="#666666" style="padding-left:10px">
			<font size=4 color="#FFFFFF"><strong>Recent Update</strong></font></td>
            <td width="548" height="45" align="right" bgcolor="#666666" style="padding-right:10px">
                <?php
//REMEMBER TO CONNECT TO DATABASE!
include 'dbconnect.php';

$q = "SELECT count(*) as cnt FROM  inventory.item_info;";
$t = mysql_query($q);
if(!$t)	die("Error quering the Database: " . mysql_error());
$a = mysql_fetch_object($t);
$total_items = $a->cnt;
if ($total_items == 0 )echo("<font size=4>There Is No Item In Stock</font>");
	...
	if(!$t)	die("Error quering the Database: " . mysql_error());
	$a = mysql_fetch_object($t);
	$total_avail_items = $a->cnt;
	echo("<em><font size=3 color=#FFFFFF><strong>$total_avail_items</strong></font><font color=#FFFFFF> Item(s) Available in Stock</font> | <font size=3 color=#FFFFFF><strong>$total_items</strong></font><font color=#FFFFFF> Item(s) in total</font></em>");
}
?>            </td>
          </tr>
        </table>
        <table width="760" height="18" border="0" cellpadding="0" cellspacing="0" style="margin-bottom:0px">
          <tr>
...
?>
		...
		<?php 
include 'dbconnect.php';
$sql="select * from inventory.item_history ORDER BY hiid DESC LIMIT $set_limit, $limit";
$result = mysql_query($sql);
if (!$result) die('Invalid query: ' . mysql_error());
...
{
	?>
<table width="760" height="60" border="0" cellpadding="0" cellspacing="0" bgcolor="#F9F8FE" style="border-bottom:1px dotted  #CCCCCC; margin-bottom:5px">
	<tr>
	<td width="371" height="35" style="padding-left:8px">
	<?php
	$iid=$arr['iid'];
	...
	?>	</td>
	<td width="206" height="35" >
	<?php
		if($arr['takereturn']=='in')
		{
			echo "<font color='green' size=3>Available Now</font>";
		}
		else if(($arr['takereturn']=='out')&&($flag[$iid]==0))
		{
			if($row1['tag']=='R')
			{
				echo "<font color='Red' size=3>Request for Reservation</font>";
				$flag[$iid]=1;
			}
			if($row1['tag']=='N'||$row1['tag']=='Y')
			{
	...
	?>	</td>
	<td width="183" height="35" style="padding-right:10px" align="right">
	<?php
	if($arr['takereturn']=='in') echo "Returned On ".$arr['trdate'];
	else echo "From ".$arr['trdate'];
	?>	</td>
	</tr>
	<tr>
	<td height="35" style="padding-left:8px; padding-bottom:5px">
	<?php
	if($arr['takereturn']=='in')
...
//	else echo "<strong><font color=#336633>Returned :)</font></strong>"; 
	?>
	</td>
	</tr>
</table>
