...
<div id="replyInfoDiv<?php echo($memofid) ?>" class="replyInfoDiv<?php echo($memofid) ?>" style=" padding-top:10; padding-bottom:0; width:450; border-bottom:1px dashed #DDDDDD">
<table width="450" border="0" cellpadding="0" cellspacing="0" bgcolor="#F5F5F5" style="margin-bottom:0; border-bottom:0 dashed #DDDDDD">
	<tr>
	<td width="478" height="25" align="left" valign="top" style=" font-weight:normal; padding-left:5; font-size:12px; padding-top:5; padding-bottom:2; line-height:130%">
	<?php 
	if($memo_follow_rstatus=="N"&&$recipient==$uname){
		if($orig_sender==$recipient)
			echo("<strong>$descrip</strong>");
		else
			echo("<strong><font color=$_SESSION[hcolor]>@$sender_fname $recipient_lname</font> $descrip</strong>");
	}else{
		if($orig_sender==$recipient)
			echo($descrip);
		else
	...
	<tr>
	  <td height="26" align="left" valign="top" style=" font-weight:normal; font-size:10px; padding:3 5 5 5; color:#999999; line-height:150%">
	  ...
	  <?php if($uname==$sender){ ?>&nbsp;&nbsp;<span style=" height:6; padding:1 5 1 5; background: #F5F5F5; width:50; display:inline; border:1px solid #DDDDDD; font-size:10px; cursor:pointer; color:#999999" align="center" id="deleteReplyBtn<?php echo($memofid) ?>" class="deleteReplyBtn<?php echo($memofid) ?>">Delete</span>
...
<textarea name="replyContent" id="replyContent<?php echo($memofid) ?>" style=" width:450; border:1px solid #DDDDDD; height:100px; font-size:13px; padding:4; font-weight:normal; font-family: Arial, Helvetica, sans-serif; margin-bottom:10px"></textarea><br />
			  <div class="replySubmitBtn<?php echo($memofid) ?>" id="replySubmitBtn<?php echo($memofid) ?>" style=" height:15; padding:2 4 2 4; background: url(img/black_cell_bg.jpg); display:inline; margin-top:10; margin-bottom: width:60; border:1px solid #333333; font-size:11px; cursor:pointer; font-weight:bold; color:#FFFFFF" align="center">Submit</div>&nbsp;
			<div class="replyCancelBtn<?php echo($memofid) ?>" id="replyCancelBtn<?php echo($memofid) ?>" style=" height:15; padding:2 4 2 4; background: url(img/master.png); display:inline; margin-top:10; width:70; border:1px solid #CCCCCC; border-top:0px solid #DDDDDD; font-size:11px; font-weight:bold; cursor:pointer" align="center">Next time</div>
                    </form>
</div>
